...
class Solver(qstls.Solver):
    """
    Class used to solve the Qstls-PIMC scheme.
    """

    native_scheme_cls = native.QstlsPimc
    ...
    def find_fixed_adr_in_database(self, inputs: "Input"):
        """
        Reuses fixed ADR only when the state point and PIMC switching inputs match.
        """
...
class Input(qstls.Input):
    """
    Class used to manage the input for the :obj:`qupled.qstlspimc.QstlsPimc` class.
    """

    theory: str = "QSTLS-PIMC"
...
class Result(stls.Result):
    """
    Results for the :obj:`qupled.qstlspimc.QstlsPimc` class.
    """

    f0_grid: any = None
    """Grid on which the constructed interacting momentum distribution is sampled."""
    f0_values: any = None
    """Constructed interacting momentum distribution sampled on ``f0_grid``."""
