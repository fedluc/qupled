...
StatePointGrid<Scheme>::StatePointGrid(
    const std::shared_ptr<const InputType> &in)
    : inPtr(in),
      alpha(numUtil::Inf),
  ...
  // Clamp so that rs - drs >= 0 and theta - dTheta >= 0
  ...
  for (const auto tOff :
    ...
    const double thetaTmp = theta + static_cast<int>(tOff) * dTheta;
    for (const auto rOff :
         {GridPoint::Rs::DOWN, GridPoint::Rs::CENTER, GridPoint::Rs::UP}) {
      const double rsTmp = rs + static_cast<int>(rOff) * drs;
      auto inTmp = std::make_shared<InputType>(*in);
      ...
      workers[idx]       = std::make_unique<Scheme>(inTmp, false, gp);
    }
  }
  setupDerivativeData();
}
...
template <typename Scheme>
void StatePointGrid<Scheme>::setupDerivativeData() {
...
template <typename Scheme>
int StatePointGrid<Scheme>::compute() {
  // Initialise each worker exactly once
  if (!initDone) {
    for (auto &w : workers) w->doInit();
    initDone = true;
  }
  // Initial guess
  for (auto &w : workers) w->doInitialGuess();
...
void StatePointGrid<Scheme>::computeLfc() {
  ...
  for (size_t i = 0; i < N; ++i) workers[i]->applyLfcDiff(lfcDerivatives[i]);
}

template <typename Scheme>
void StatePointGrid<Scheme>::computeLfcDerivatives() {
  assert(alpha != numUtil::Inf);
  const double drs    = inPtr->getCouplingResolution();
  const double dTheta = inPtr->getDegeneracyResolution();
  const double dx     = inPtr->getWaveVectorGridRes();
  ...
  for (size_t i = 0; i < N; ++i) {
    ...
    for (size_t l = 0; l < lfc.size(1); ++l) {
      ...
      lfcd(0, l) = a_dx * wvg[0] * derivative(lfc, l, 0, DerivativeData::Type::FORWARD);
      for (size_t k = 1; k < nx - 1; ++k) {
        lfcd(k, l) = a_dx * wvg[k] * derivative(lfc, l, k, DerivativeData::Type::CENTERED);
      }
      lfcd(nx - 1, l) =
          a_dx * wvg[nx - 1]
...
template <typename Scheme>
double StatePointGrid<Scheme>::derivative(const Vector2D &f,
                                          int l,
                                          size_t i,
...
template <typename Scheme>
double StatePointGrid<Scheme>::derivative(double f0,
                                          double f1,
                                          double f2,
                                          DerivativeData::Type t) const {
  switch (t) {
...
template <typename Scheme>
const std::vector<double> &StatePointGrid<Scheme>::getSsf(GridPoint p) const {
  return workers[p.toIndex()]->getSsf();
}

template <typename Scheme>
const Vector2D &StatePointGrid<Scheme>::getLfc(GridPoint p) const {
  return workers[p.toIndex()]->getLfc();
}

template <typename Scheme>
const std::vector<double> &StatePointGrid<Scheme>::getWvg(GridPoint p) const {
  return workers[p.toIndex()]->getWvg();
}

template <typename Scheme>
double StatePointGrid<Scheme>::getCoupling(GridPoint p) const {
  return rsValues[p.toIndex()];
}

template <typename Scheme>
double StatePointGrid<Scheme>::getDegeneracy(GridPoint p) const {
  return thetaValues[p.toIndex()];
}

template <typename Scheme>
double StatePointGrid<Scheme>::getUInt(GridPoint p) const {
...
template <typename Scheme>
double StatePointGrid<Scheme>::getFxcIntegrandValue(GridPoint p) const {
  const size_t i = p.toIndex();
  return thermoUtil::computeInternalEnergy(workers[i]->getWvg(),
                                           workers[i]->getSsf(),
                                           1.0,
...
template <typename Scheme>
const Scheme &StatePointGrid<Scheme>::getWorkerAt(GridPoint p) const {
  return *workers[p.toIndex()];
}

template <typename Scheme>
const Scheme &StatePointGrid<Scheme>::centralWorker() const {
  return *workers[CENTER.toIndex()];
}

// Explicit instantiations (keeps template implementation out of headers)
