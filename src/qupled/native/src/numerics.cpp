...
AutoDiff2D AutoDiff2D::operator+(const AutoDiff2D &other) const {
...
AutoDiff2D AutoDiff2D::operator+(double scalar) const {
  return AutoDiff2D(val + scalar, dx, dy, dxx, dyy, dxy);
}

AutoDiff2D operator+(double scalar, const AutoDiff2D &dual) {
  return dual + scalar;
}
...
AutoDiff2D exp(const AutoDiff2D &x) {
...
AutoDiff2D sqrt(const AutoDiff2D &x) {
...
AutoDiff2D tanh(const AutoDiff2D &x) {
