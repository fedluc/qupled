...
double CSR::getInternalEnergy() const {
  const double rs = inRpa().getCoupling();
  return thermoUtil::computeInternalEnergy(getWvg(), getSsf(), rs, inRpa().getDimension());
}

double CSR::getFreeEnergyIntegrand() const {
  return thermoUtil::computeInternalEnergy(getWvg(), getSsf(), 1.0, inRpa().getDimension());
}
