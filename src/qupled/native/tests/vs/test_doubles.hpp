...
namespace vsTestDoubles {
  ...
  public:
    ...
    void setCenterError(double value) {
      worker(GridPoints::CENTER).setError(value);
    }
    ...
    void buildWorkersAndGridMetadata() {
      ...
      for (const auto theta_off : {GridPoint::Theta::DOWN,
        ...
        for (const auto rs_off :
          ...
          workers[idx] =
              std::make_unique<FakeVSWorker>(rs,
                                             theta,
                                             rs,
  ...
  inline std::shared_ptr<VSStlsInput>
  makeVsInput(const double coupling = 1.0, const double degeneracy = 1.0) {
