...
TEST(StlsApiAndUtilTest, ComputeSupportsDefaultGuess) {
...
TEST(StlsApiAndUtilTest, ComputeSupportsUserProvidedGuess) {
...
TEST(StlsApiAndUtilTest, DynamicPointerCastHelperSucceedsForDerivedInput) {
...
TEST(StlsApiAndUtilTest, DynamicPointerCastHelperThrowsForIncompatibleInput) {
...
TEST(StlsApiAndUtilTest, SlfcUtilityReturnsZeroAtXZeroIn3D) {
...
TEST(StlsApiAndUtilTest, SlfcUtilityReturnsZeroAtXZeroIn2D) {
  const std::vector<double> wvg{0.0, 0.5, 1.0, 1.5, 2.0};
  const std::vector<double> ssf(wvg.size(), 1.0);
  auto ssfi = std::make_shared<Interpolator1D>(wvg, ssf);
  auto itg = std::make_shared<Integrator1D>(1.0e-8);
...
TEST(StlsApiAndUtilTest, SlfcUtilityReturnsFiniteValueAtNonzeroXIn3D) {
  const std::vector<double> wvg{0.0, 0.5, 1.0, 1.5, 2.0};
  const std::vector<double> ssf(wvg.size(), 1.0);
  auto ssfi = std::make_shared<Interpolator1D>(wvg, ssf);
  auto itg = std::make_shared<Integrator1D>(1.0e-8);
