...
@pytest.fixture
def cf():
    return CorrelationFunctions("test.db")


@pytest.mark.unit
def test_initialization():
    cf = CorrelationFunctions("my.db")
    assert cf.database_name == "my.db"


@pytest.mark.unit
def test_initialization_default_database():
    cf = CorrelationFunctions()
    assert cf.database_name is None


@pytest.mark.unit
def test_compute_rdf_with_default_grid(mocker, cf, read_run, db_handler, scheme_tables):
...
@pytest.mark.unit
def test_compute_rdf_passes_custom_grid(mocker, cf, read_run, db_handler):
    mock_data = mocker.Mock()
    mock_data.inputs = {"dimension": {"value": "_3D"}}
    mock_data.results = {}
    read_run.return_value = mock_data
    ...
    cf.compute_rdf(run_id=2, rdf_grid=custom_grid)
    args = mock_result.compute_rdf.call_args[0]
    assert np.allclose(args[1], custom_grid)


@pytest.mark.unit
def test_compute_itcf_hf_theory(mocker, cf, read_run, db_handler, scheme_tables):
    mock_data = mocker.Mock()
    mock_data.inputs = {}
    mock_data.results = {}
...
@pytest.mark.unit
def test_compute_itcf_rpa_theory(mocker, cf, read_run, db_handler, scheme_tables):
    mock_data = mocker.Mock()
    mock_data.inputs = {}
    mock_data.results = {}
...
@pytest.mark.unit
def test_compute_itcf_passes_custom_tau(mocker, cf, read_run, db_handler):
    mock_data = mocker.Mock()
    mock_data.inputs = {}
    mock_data.results = {}
    mock_data.run = {"theory": "HF"}
    ...
    cf.compute_itcf(run_id=3, tau=custom_tau)
...
def test_compute_itcf_stores_results_from_result_object(
    mocker, cf, read_run, db_handler, scheme_tables
):
