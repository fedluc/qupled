...
def test_inspect_runs(db_handler):
    db_handler.return_value.inspect_scheme_runs.return_value = {"run1": "data1"}
...
def test_read_run(db_handler):
    db_handler.return_value.get_scheme_run.return_value = {
        "input1": "data1",
        "result1": "data2",
    }
...
def test_read_inputs(db_handler):
    db_handler.return_value.get_scheme_inputs.return_value = {"input1": "data1"}
...
def test_read_results(db_handler):
    db_handler.return_value.get_scheme_results.return_value = {"result1": "data1"}
