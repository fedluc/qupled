...
def test_default(qstls_instance):
    ...
    assert all(x == y for x, y in zip(qstls_instance.inputs.chemicalPotential, [-10.0, 10.0]))
...
def test_set_input():
    qstls = Qstls(2.0, 0.5, [-5, 5], 20, 1.0e-8, "fixedAdrFile", 0.5, None,
                  100, 32, 100, "recoveryFile", 0.01, "segregated", 16)
...
def test_setGuess(qstls_instance, mocker):
    arr = np.ones(10)
    mockHdfRead = mocker.patch("qupled.util.Hdf.read", return_value={"wvg" : arr, "ssf" : arr})
